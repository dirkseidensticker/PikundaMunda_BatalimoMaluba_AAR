...
osm.rivers.lines <- geojsonsf::geojson_sf("data/gis/OSM_river_lines.geojson")
osm.rivers.poly <- geojsonsf::geojson_sf("data/gis/OSM_river_lakes_poly.geojson") %>%
  sf::st_make_valid()
osm.coast.line <- geojsonsf::geojson_sf("data/gis/OSM_coast_lines.geojson")
...
p <- ggplot() + 
  ...
  annotate("text", x = 16, y = 0, label = "Rep. Congo", fontface  = "bold", colour = "#485063") + 
